...
// A szerver nem feltétlenül magyar idozonaban fut (pl. Vercel altalaban UTC-t hasznal),
// ezert a "naptar feed"-hez ki kell szamolnunk pontosan, mi az adott magyar helyi ido
// (Europe/Budapest, nyari/teli idoszamitassal egyutt) UTC megfeleloje.
function budapestOffsetMinutes(utcGuess) {
...
export function budapestLocalToUTC(y, m, d, hour, minute) {
  const guess = new Date(Date.UTC(y, m - 1, d, hour, minute));
  const offsetMin = budapestOffsetMinutes(guess);
